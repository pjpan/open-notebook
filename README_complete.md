...
# Open Notebook 项目结构整理

## 项目概览
Open Notebook 是一个开源的、隐私优先的 AI 研究助理，支持多模态内容上传（PDF、音频、视频、网页）、智能笔记生成、语义搜索、AI 聊天和播客制作。项目采用三层架构：

- **前端**：Next.js + TypeScript，提供用户界面
- **API 后端**：FastAPI + Python，处理业务逻辑和数据存储
- **数据库**：Supabase（PostgreSQL + pgvector），用于数据存储和语义搜索

## 目录结构

```
/Users/pengju.pan/gitcode/open-notebook/
├── api/                          # FastAPI 后端服务
│   ├── CLAUDE.md                 # API 架构文档
│   ├── main.py                   # 主入口文件
│   ├── client.py                 # API 客户端
...
│   ├── episode_profiles_service.py  # 播客集配置服务
│   ├── insights_service.py       # 洞察服务
│   ├── models_service.py         # 模型服务
│   ├── notebook_service.py       # 笔记本服务
│   ├── notes_service.py          # 笔记服务
│   ├── podcast_api_service.py    # 播客 API 服务
│   ├── podcast_service.py        # 播客服务
│   ├── search_service.py         # 搜索服务
│   ├── settings_service.py       # 设置服务
│   ├── sources_service.py        # 源服务
│   ├── transformations_service.py  # 转换服务
│   └── routers/                  # API 路由（23 个文件）
├── frontend/                     # Next.js 前端
│   ├── package.json              # 项目依赖
...
│   ├── public/                   # 静态资源
│   └── src/                      # 源代码（组件、页面、状态管理）
├── open_notebook/                # 核心业务逻辑层
│   ├── CLAUDE.md                 # 核心逻辑架构文档
│   ├── config.py                 # 配置文件
│   ├── exceptions.py             # 异常定义
│   ├── ai/                       # AI 模型管理（7 个文件）
│   ├── database/                 # 数据库操作（7 个文件）
│   ├── domain/                   # 领域模型（9 个文件）
│   ├── graphs/                   # LangGraph 工作流（10 个文件）
│   ├── podcasts/                 # 播客生成（5 个文件）
│   └── utils/                    # 工具函数（9 个文件）
├── tests/                        # 测试代码（8 个文件）
├── docs/                         # 文档
...
- **AI 提供商**：Esperanto 库（支持 8+ 提供商）
- **任务队列**：Supabase 异步任务队列
- **日志**：Loguru
- **验证**：Pydantic v2
- **测试**：Pytest

### 数据库
- **Supabase**：PostgreSQL + pgvector（向量搜索）
- **迁移**：AsyncMigrationManager（自动迁移）

### 其他服务
- **内容处理**：content-core 库（文件/URL 提取）
- **提示管理**：AI-Prompter（Jinja2 模板）
- **播客生成**：podcast-creator 库
- **嵌入生成**：Esperanto（多提供商）

## 架构亮点

1. **异步优先设计**：所有数据库查询、图调用和 API 调用均为异步（await）
2. **LangGraph 工作流**：
   - source.py：内容摄入（提取 → 嵌入 → 保存）
   - chat.py：会话代理（消息历史）
   - ask.py：搜索 + 合成（检索相关源 → LLM）
   - transformation.py：源自定义转换
3. **多提供商 AI**：Esperanto 库提供统一接口，支持 8+ AI 提供商
4. **数据库架构**：自动迁移、关系模型、向量搜索、ACID 事务
5. **播客生成**：异步任务队列，支持轮询状态

## 快速启动

1. **环境变量**：复制 `.env.example` 为 `.env` 并配置
2. **依赖安装**：
   - 后端：`uv pip install -r requirements.txt`
   - 前端：`cd frontend && npm install`
3. **运行服务**：
   - 后端：`uv run python run_api.py`（端口 5055）
   - 前端：`cd frontend && npm run dev`（端口 3000）
4. **数据库**：确保 Supabase 服务运行（本地或云端）

## 常用命令

- **运行测试**：`uv run pytest tests/`
- **构建 Docker 镜像**：`make docker-build`
- **启动全栈服务**：`docker compose --profile multi up`

## 项目文档

- **架构指南**：/CLAUDE.md、/api/CLAUDE.md、/open_notebook/CLAUDE.md
- **用户文档**：/docs/index.md
- **配置说明**：/CONFIGURATION.md
- **贡献指南**：/CONTRIBUTING.md
- **部署指南**：/MAINTAINER_GUIDE.md

**项目版本**：1.2.4+ | **最后更新**：2026-01-21