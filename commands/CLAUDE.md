...
**Purpose**: Defines async command handlers for long-running operations via Supabase job queue system.
...
- **Error handling**: Permanent errors return failure output; `RuntimeError` exceptions auto-retry via Supabase job queue.
- **Retry configuration**: Aggressive retry settings (15 attempts, 1-120s backoff, DEBUG log level) are used for handling database transaction conflicts.
...
**External**: `loguru`, `pydantic`, `podcast_creator`, Supabase job queue system
...
- **podcast_commands**: Profiles loaded from Supabase by name (must exist); briefing can be extended with suffix. Episode records created mid-execution.
