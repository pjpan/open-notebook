...
Encapsulates podcast metadata and configuration: speaker profiles (voice/personality config), episode profiles (generation settings), and podcast episodes (with job status tracking via supabase job queue).
...
All inherit from `ObjectModel` (Supabase base class with table_name and save/load).
...
- **Job tracking**: command field links to supabase job queue ID
- `get_job_status()` fetches async job status via supabase job queue library
- `_prepare_save_data()` ensures command field is always string format for database
...
- **Job tracking**: command field delegates to supabase job queue; get_job_status() returns "unknown" on failure
- **Job ID handling**: string IDs used for job references
...
- `open_notebook.database.job_queue` (optional): get_command_status() for job status
...
4. **Change job provider**: Replace supabase job queue with alternative job queue library; update get_job_status()
